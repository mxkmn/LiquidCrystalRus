...
// LiquidCrystalRus constructor is called).
...
void LiquidCrystalRus::setRowOffsets(int row0, int row1, int row2, int row3)
{
  _row_offsets[0] = row0;
  _row_offsets[1] = row1;
  _row_offsets[2] = row2;
...
inline size_t LiquidCrystalRus::write(uint8_t value) {
  if (value >= 0x80) { // UTF-8 handling
    if (value >= 0xc0) utf_hi_char = value - 0xd0;
    else {
      uint8_t mod_char = value & 0x3f;
      if (!utf_hi_char && (mod_char == 1)) send(0xa2,HIGH); // Ё
      else if ((utf_hi_char == 1) && (mod_char == 0x11)) send(0xb5,HIGH); // ё
      else send(pgm_read_byte_near(utf_recode + mod_char + (utf_hi_char<<6) - 0x10), HIGH);
    }    
  }
  else send(value, HIGH);
  return 1; // assume sucess
}

inline size_t LiquidCrystalRus::writeRaw(uint8_t value) {
  send(value, HIGH);
  return 1; // assume sucess
}
...
void LiquidCrystalRus::pulseEnable(void) {
  digitalWrite(_enable_pin, LOW);
  delayMicroseconds(1);    
...
void LiquidCrystalRus::write8bits(uint8_t value) {
  for (int i = 0; i < 8; i++) {
    digitalWrite(_data_pins[i], (value >> i) & 0x01);
  }
